...
class CategoryOptionTempMapper extends React.Component {
  ...
  render() {
    ...
    }, tran.translate('CATEGORY_OPTION_ADD_HEADER')), React.createElement("br", null), React.createElement("label", {
      className: "g-line-height-1_8 g-letter-spacing-1  g-mb-20 form-control-label"
    }, tran.translate('CATEGORY_OPTION_ADD_HEADER_TEXT'))), this.state.catOption.category_link != undefined && this.state.catOption.category_link.length > 0 ? React.createElement(Col, {
      className: "g-mb-50"
    }, React.createElement(Collapsible, {
      trigger: tran.translate('SHOW_DEPENDENCIES_CATEGORIES') + " - " + this.state.catOption.category_link.length
    }, React.createElement("div", null, this.state.catOption.category_link.map(item => {
      return React.createElement(Link, {
        to: "/categories/edit/" + item.category_id,
        className: "g-font-size-11 col-md-11 col-xs-11 g-color-primary--hover nav-link"
      }, item.category["category_" + this.props.lang]);
    })))) : React.createElement("span", null), React.createElement(DropDownList, {
      isRequired: true,
    ...
    }), this.state.catOption.cat_opt.id ? this.checkType('SELECT').length > 0 ? React.createElement("div", null, React.createElement(CategoryOptionSELECT, {
      catOptionsTemp: this.checkType('SELECT')[0],
      catOption: this.props.catOptions.catOptions.filter(item => {
        return item.id == this.state.id;
      })[0]
    })) : this.checkType('MULTI_SELECT').length > 0 ? React.createElement("div", null, React.createElement(CategoryOptionSELECT, {
      catOptionsTemp: this.checkType('MULTI_SELECT')[0],
      catOption: this.props.catOptions.catOptions.filter(item => {
        return item.id == this.state.id;
      })[0]
    })) : this.checkType('SINGLE').length > 0 ? React.createElement("div", null, React.createElement(CategoryOptionSINGLE, {
      catOptionsTemp: this.checkType('SINGLE')[0],
      catOption: this.props.catOptions.catOptions.filter(item => {
        return item.id == this.state.id;
      })[0]
    })) : this.checkType('BETWEEN').length > 0 ? React.createElement("div", null, React.createElement(CategoryOptionBETWEEN, {
      catOptionsTemp: this.checkType('BETWEEN')[0],
      catOption: this.props.catOptions.catOptions.filter(item => {
        return item.id == this.state.id;
      })[0]
    })) : this.checkType('GEOCOORDINATE').length > 0 ? React.createElement("div", null, React.createElement(CategoryOptionGEO, {
      catOptionsTemp: this.checkType('GEOCOORDINATE')[0],
      catOption: this.props.catOptions.catOptions.filter(item => {
        return item.id == this.state.id;
      })[0]
    })) : this.checkType('IMAGES').length > 0 ? React.createElement("div", null, React.createElement(CategoryOptionIMAGE, {
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    getCategoryOption: id => {
      return dispatch(new BaseService().queryThunt(QueryList.CategoryOptions.GET_ALL_CETEGORIES_OPTIONS, {
        id: id
      }));
    }
  };
...
//# sourceMappingURL=CategoryOptionTempMapper.js.map