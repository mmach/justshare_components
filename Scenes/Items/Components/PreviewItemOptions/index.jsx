...
class PreviewItemOptions extends React.Component {
    ...
    singleValue(item) {
        return <span className="g-pa-2 g-my-2">{item.value}</span>
    }
    selectValue(item) {
        return <span className="g-pa-2 g-my-2">{item.cat_opt_temp["value_" + this.props.lang]}</span>
    }
    ...
    render() {
        ...
        return (
            Object.keys(groupByCat).sort((a, b) => {
                ...
                console.log(item);
                return <Col className="g-my-2 g-py-0 " xs={this.props.col_size}>
                    <label className="g-my-0 g-py-0 g-color-gray-dark-v4 g-letter-spacing-1  g-line-height-0_8 g-font-weight-600 g-font-size-9">{groupByCat[item].catOption["name_" + this.props.lang]}</label>
                    {
                        this.props.item.itemCategoryOption.sort((a,b)=>{
                        return Number(a.cat_opt_temp.order)>Number(b.cat_opt_temp.order)?1:-1
                        }).filter(option => { return option.category_link.co_id == item && option.cat_opt_temp.is_visible_view!=true}).map(result => {
                            return <span className=" g-line-height-0_8 g-my-0 g-py-0" style={{display:'block',marginTop:'-5px'}}>{this.showValue(result)}</span>
                        })
                    }
