...
class CategoryFilter extends React.Component {
    ...
    componentDidMount() {
        this.props.getCategories({ parent: this.props.category_id ? this.props.category_id : '_ROOT' }).then(succ => {
    ...
    render() {
        ...
        if (this.state.isLoading) {
            return (<BodyLoader zIndex={3} height="600px" size="100px" progress={this.props.loader.BODY_PROGRESS} />);
        }
        return (



            <div class="list-group list-group-border-0 g-mb-10">
                ...
                {this.state.categoriesList[0].category_parent.map(item => {
                    ...
                    return <span data-tag={(item.category_parent != undefined ? item.category_parent.length : 0) > 0 ? item.category_parent[0].id : '_ROOT'} onClick={this.getCategories.bind(this)} className=" list-group-item list-group-item-action justify-content-between u-link-v5 g-cursor-pointer    g-pl-7--hover ">
                        <span className="g-line-height-1 g-letter-spacing-1 g-font-weight-500 g-font-size-12  text-uppercase">
                            <span class="  g-font-size-15 g-font-weight-600 g-pr-10 " aria-hidden="true">...</span>{item.category_parent[0]["category_" + this.props.lang]}</span>
                    </span>
                   ...
                   {this.state.categoriesList[0].category_parent.map(item => {
                    ...
                    return <span data-tag={item.id} onClick={this.getCategories.bind(this)} className=" list-group-item list-group-item-action justify-content-between u-link-v5 g-cursor-pointer    g-pl-7--hover ">
                        <span className="g-line-height-1 g-letter-spacing-1 g-font-weight-500 g-font-size-12  text-uppercase">
                            <span class="  g-font-size-15 g-font-weight-600 g-pr-10 " aria-hidden="true">.</span>{item["category_" + this.props.lang]}</span>
                    </span>
                ...
                {this.state.categoriesList.filter(item => { return item.category_children.length > 0 }).map(item => {
                    ...
                    if (this.props.aggs.categories != undefined) {
                        let result = this.props.aggs.categories.buckets.filter(cat => { return cat.key == item.id })
                        if (result.length == 1) {
                    ...
                    return <span data-tag={item.id} onClick={this.getCategories.bind(this)} className=" list-group-item list-group-item-action justify-content-between u-link-v5 g-cursor-pointer    g-pl-7--hover ">
                        ...
                        {counter > -1 ? < span class="u-label g-rounded-20 g-px-15  g-mr-10 " style={{ backgroundColor: item.color }}>{this.props.isLoading == true ? <span><i class="fa fa-spinner fa-spin" /></span> : <span>{counter}</span>}</span> : <span></span>}
                    </span>
                ...
                {this.state.categoriesList.filter(item => { return item.category_children.length == 0 }).map(item => {
                    ...
                    return <span data-tag={item.id} style={{ color: '#333' }} onClick={this.getCategories.bind(this)} className=" list-group-item list-group-item-action justify-content-between u-link-v5 g-cursor-pointer    g-pl-7--hover ">
